...
'$ GenerateDate: 07/21/2025 12:33:29
    ...
    Public Class [ADM_Voltage_Master]
    ...
    Private this as ADM_Voltage_Master = me
    ...
    AddProperty("704", "PartNumber", <a><![CDATA[Part Number]]></a>.Value, "", "String","N","System","MN", 9999, "", 0,0, "", "", "GLOBAL\H601424", "6/19/2025 7:43:08 PM")
      ...
      oSubpart = AddSubpart(70,"Rows", <a><![CDATA[Rows]]></a>.Value, "MN", "Subpart for All Data Rows for this Master", "Master SubPart Collection", 9999, "", "GLOBAL\H601422", "6/25/2025 4:10:37 AM")
      
        oSubpart.AddVPF (52, "ADM_Voltage_Master_Row", "ADM_Voltage_Master_Row")
    ...
    ctx = ContextId
          ...
          InitProperty("Row_DBKeys", "656", "", "", "Y", "","", 0, "-1", 0, "", "N","0",  "GLOBAL\H601424", "6/20/2025 7:23:32 PM", "DataBase Extract of Key Values for Lines", "In Development",  0,1622)
        End If
            If Incontext("-1", ctx) Then
    ...
    ctx = ContextId
        ...
        InitValidValue("Row_DBKeys_ValidValues", "656", "-1", 1588)
        End If
    End Sub
    ...
    #Region " Formulas "
      ...
      Stop
      End If
      '   BEGIN FORMULA; PROP ID:656; TYPE:PF
      Result = Me.ValidValues("Row_DBKeys").Count
      '   END FORMULA; PROP ID:656; TYPE:PF
      Catch ex As Exception
      ObjectManager.LogError("Application: " + Me.Application + " ADM_Voltage_Master.Formula_Row_DBKeys", ex.Message)
      ...
      Stop
      End If
      '   BEGIN FORMULA; PROP ID:657; TYPE:PF
      result = ""
      '   END FORMULA; PROP ID:657; TYPE:PF
      Catch ex As Exception
      ObjectManager.LogError("Application: " + Me.Application + " ADM_Voltage_Master.Formula_Data_Hash", ex.Message)
      If ObjectManager.StopOnErrors Then Stop
      End Try
      Return Result
      End Function
      ...
      Stop
      End If
      '   BEGIN FORMULA; PROP ID:1430; TYPE:PF
      Result = False
If (Me.Rows.Count > 0) Then
	...
	End If
End If
      '   END FORMULA; PROP ID:1430; TYPE:PF
      Catch ex As Exception
      ObjectManager.LogError("Application: " + Me.Application + " ADM_Voltage_Master.Formula_Is_Dirty", ex.Message)
      If ObjectManager.StopOnErrors Then Stop
      End Try
      Return Result
      End Function
      ...
      Stop
      End If
      '   BEGIN FORMULA; PROP ID:658; TYPE:PF
      result = -1
      '   END FORMULA; PROP ID:658; TYPE:PF
      Catch ex As Exception
      ObjectManager.LogError("Application: " + Me.Application + " ADM_Voltage_Master.Formula_Selected_Row", ex.Message)
      If ObjectManager.StopOnErrors Then Stop
      End Try
      Return Result
      End Function
      ...
      Stop
      End If
      '   BEGIN FORMULA; PROP ID:656; TYPE:VV
...
Result = MakeValidValueKeyFromDatabase($"[{TableName}]", $"[{PK_Name}]", $"[{PK_Name}]", $"WHERE [{PK_Name}] > " & FormatCriteria(0) & $" ORDER BY [{PK_Name}]")
      '   END FORMULA; PROP ID:656; TYPE:VV
      Catch ex As Exception
      ObjectManager.LogError("Application: " + Me.Application + " ADM_Voltage_Master.Formula_Row_DBKeys_ValidValues", ex.Message)
      If ObjectManager.StopOnErrors Then Stop
      End Try
      Return Result
      End Function
