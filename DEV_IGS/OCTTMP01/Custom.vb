...
#Region "XML / JSON Conversions"
    ...
    End Function
    Public Function GetPFJSONRepresentation(ByRef MyProperty As PropertySF, Optional IngorePartNumber As Boolean = True) As String
        ...
        End If
        Const qt As String = """"
        If MyPF.Properties.ContainsKey("JSONObjectName") Then
            JSONObjectName = (MyPF.Properties("JSONObjectName").DisplayValue)
            MyProps.Append("{")
            ' Dim tmpResult As String = ""
            Dim isFirst As Boolean = True  ' Flag To track the first item

            For Each p As PropertySF In MyPF.Properties
                If p.Name.ToUpper() <> MyName.ToUpper() AndAlso Not gc_ExcludePropsFromXMLJSON.ToUpper().Contains(String.Concat(p.Name, ";").ToUpper()) Then
                    If Not isFirst Then
                        MyProps.Append(", ")  '// Add comma And space only If it's not the first item

                    End If
                    MyProps.Append(String.Concat($"{qt}{p.Name}{qt}:{qt}", p.DisplayValue, $"{qt}"))
                    isFirst = False ' Set the flag To False after the first item
                End If
            Next p
            ...
            ' MyProps.Append(String.Concat($"{qt}{p.Name}{qt}{qt}:", p.DisplayValue, $"{qt}"))
            ' If Debug_Mode Then
            ' ' Console.WriteLine($"{qt}{p.Name}{qt} : {qt}{p.DisplayValue}{qt}")
            ' End If
            ' End If
            '   Next p
            MyProps.Append("}")
            For Each s As Subpart In MyPF.SubParts
                For Each pf As Part In s.Parts()
                    If pf.Properties.ContainsKey(MyProperty.Name) Then
                        MyProps.Append(GetPFJSONRepresentation(pf.Properties(MyProperty.Name)))
                    End If
                Next pf
            Next
            Result = String.Concat($"{{{JSONObjectName} : ", MyProps.ToString(), $"}}")
        End If
